...
class AnalysisOutput(models.Model):
    ...
    analysis_type = models.CharField(
        max_length=50, choices=ANALYSIS_TYPE_CHOICES, default="bulk_rna"
    )
    ...
    linked_benchling_entry = models.CharField(
        max_length=255, null=True, blank=True
    )  # Assuming Benchling entry is referenced via an ID or URL
...
class GeneCollection(models.Model):
    ...
    created_by = models.ForeignKey(
        ShopifyUserSession, on_delete=models.SET_NULL, null=True, blank=True
    )
...
class Tier(models.Model):
    ...
    )  # Optional field for tier details
...
class UserTier(models.Model):
    shopify_session = models.OneToOneField(ShopifyUserSession, on_delete=models.CASCADE)
    tier = models.ForeignKey(Tier, on_delete=models.CASCADE)  # Link to the Tier model

    def __str__(self):
        return f"{self.shopify_session.shopify_email} - {self.tier.name}"


class UserGeneRequest(models.Model):
    shopify_session = models.ForeignKey(ShopifyUserSession, on_delete=models.CASCADE)
    ...
    def is_within_limit(self):
        ...
        """
        try:
            user_tier = UserTier.objects.get(shopify_session=self.shopify_session)
