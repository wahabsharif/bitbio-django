...
function populateCellTypes(types) {
  ...
  const hasOtherInAPI = types.some(
    (type) => type.product_name && type.product_name.toLowerCase() === "other",
  );
...
function resetCalculator() {
  ...
  if (resultsSection) resultsSection.classList.remove("results-visible");
...
function handleInputWheelEvent(e) {
  ...
  const isViabilityField = ["viability1", "viability2", "viability3"].includes(
    this.id,
  );
...
function validateViabilitySequence(currentFieldId) {
  ...
  const viability1Value = viability1 ? parseDecimalInput(viability1.value) : 0;
  const viability2Value = viability2 ? parseDecimalInput(viability2.value) : 0;
